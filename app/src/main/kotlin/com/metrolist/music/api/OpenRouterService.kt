...
Output MUST be a JSON array with EXACTLY $lineCount strings."""
                    ...
                    // Validate the user-supplied base URL: only allow HTTPS to prevent
                    // API key leakage to attacker-controlled HTTP endpoints.
                    val safeBaseUrl = baseUrl.trim().let { url ->
                        if (url.isBlank()) {
                            "https://openrouter.ai/api/v1/chat/completions"
                        } else if (url.startsWith("https://", ignoreCase = true)) {
                            url
                        } else {
                            Timber.w("OpenRouterService: Rejecting non-HTTPS baseUrl, using default")
                            "https://openrouter.ai/api/v1/chat/completions"
                        }
                    }
